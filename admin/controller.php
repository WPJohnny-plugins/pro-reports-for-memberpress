...
function mepr_pro_report_handler() {
    ...
    $q_transactions = $wpdb->prepare("SELECT total, product_id, coupon_id, DATE_SUB(created_at, INTERVAL " . $time_zone . " HOUR) created_at, status FROM wp_mepr_transactions WHERE status=%s OR status=%s", 'complete', 'refunded');
